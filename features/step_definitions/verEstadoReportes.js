...
Given('soy un usuario autenticado en la página de mis reportes', { timeout: 60 * 1000 }, async function () {
...
When('selecciono el reporte de bug que envié',{ timeout: 60 * 1000 }, async function () {
  // Accedemos al primer hijo 
  const element = await driver.wait(
    until.elementLocated(By.css('.accordion .accordion-item .card .accordion-header .accordion-button')),
    10000 // tiempo de espera en milisegundos
  );
  await element.click();

  const body = await driver.wait(
    until.elementLocated(By.css('.accordion .accordion-item .card .accordion-collapse .accordion-body span:nth-child(2)')),
    10000 // tiempo de espera en milisegundos
  );
  const present = await driver.wait(
    until.elementIsVisible(body)
  );
