...
When("hago click en el botón bugs pendientes del menú lateral", async () => {
  const button = await driver.wait(
    until.elementLocated(By.id("radio-0")),
    10000
  );
  button.click();
...
Then("debería ver la lista con los bug no asignados", async () => {
  const lista = await driver.wait(
    until.elementLocated(By.className("lista-container")),
    10000
  );
...
When("hago click en el botón de ingresar del primer bug", async () => {
  const button = await driver.wait(
    until.elementLocated(By.id("probar-0")),
    10000
  );
  button.click();
});
...
When("doy click en enviar reporte", async () => {
  const button = await driver.wait(
    until.elementLocated(By.name("enviar-reporte")),
    10000
  );
  button.click();
});
...
Then(
  "se debe mostrar una advertencia, pidiendo rellenar todos los campos",
  async () => {
    ...
    if (!alertaPresente) {
      assert.fail(
        "No se presento alerta al ingresar reporte sin depurador asignado"
      );
    }
  }
);
...
Then("debería ver la lista con los bug en proceso", async () => {
  const lista = await driver.wait(
    until.elementLocated(By.className("acordeon-bugs-proceso")),
    10000
  );
  setTimeout(async () => {
    const listaVisible = await lista.isDisplayed();
...
When(
  "hago click en el primer reporte de la lista y en su botón de enviar reporte final",
  async () => {
    const reporte = await driver.wait(
      until.elementLocated(By.id("reporte-pendiente-0")),
      10000
    );
    reporte.click();
    const button = await driver.wait(
      until.elementLocated(By.name("enviar-reporte-final")),
      10000
    );
    button.click();
  }
...
Then(
  "se debe mostrar una advertencia, pidiendo dar detalles del avance.",
  async () => {
    const alerta = await driver.wait(
      until.elementLocated(By.className("modal-basic")),
      10000
    );
    const alertaPresente = await alerta.isDisplayed();
    if (!alertaPresente) {
      assert.fail(
        "No se presento alerta al ingresar reporte sin detalles entregados"
      );
    }
  }
);
...
When("hago click en el primer reporte de la lista de reportes", async () => {
  // try {
  const reporte = await driver.wait(
    elementIsEnabled(By.id("reporte-pendiente-0")),
    10000
  );
  await reporte.click();
...
Then("debo poder indicar los detalles de la finalizacion", async () => {
  const textarea = await driver.wait(
    until.elementLocated(By.id("reporte-textbox-0"))
  );
