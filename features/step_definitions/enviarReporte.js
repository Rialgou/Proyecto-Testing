...
Given(
  "soy un usuario autenticado en la página de nuevo reporte",
  { timeout: 60 * 1000 },
  async function () {
    ...
    const newReport = await driver.wait(
      until.elementLocated(By.name("nuevo-reporte"))
    );
...
When(
  "ingreso todos los detalles del reporte de bug válidos",
  async function () {
    // Seleccionamos un proyecto
    const selectProject = await driver.findElement(
      By.xpath("//button[text()='Seleccionar proyecto']")
    );
    await selectProject.click();
    const project = await driver.wait(
      until.elementLocated(By.xpath("//button[text()='EduQuest']"))
    );
    await project.click();
    await selectProject.click();
    // Le agregamos un titulo al reporte de bug
    const bugTitleSelector = await driver.findElement(
      By.xpath("//button[text()='Título del Bug']")
    );
    await bugTitleSelector.click();
    const bugTitle = await driver.wait(
      until.elementLocated(By.css('[data-testid="title-test"]'))
    );
    bugTitle.sendKeys(newTitle);
    // Le agregamos una descripción al reporte de bug
    const bugDescriptionSelector = await driver.findElement(
      By.xpath("//button[text()='Descripción del Bug']")
    );
    await bugDescriptionSelector.click();
    const bugDescription = await driver.wait(
      until.elementLocated(By.css('[data-testid="description-test"]'))
    );
...
When('hago clic en el botón "Enviar reporte"', async function () {
  const sendBugReport = await driver.findElement(
    By.xpath("//button[text()='Enviar reporte']")
  );
...
Then("debería ver un mensaje de éxito", async function () {
  ...
  const successModal = await driver.wait(
    until.elementLocated(By.css('[data-testid="success-modal"]'))
  );
  await driver.wait(until.elementIsVisible(successModal));
  // Encuentra el botón de cerrar dentro del modal
  const closeButton = await successModal.findElement(
    By.xpath('//button[text()="Cerrar"]')
  );
...
Then(
  "la lista de reportes debería contener el nuevo reporte",
  async function () {
    // Accedemos al primer hijo
    const element = await driver.wait(
      until.elementLocated(
        By.css(
          ".accordion .accordion-item .card .accordion-header .accordion-button div .bug-info span:last-child"
        )
      ),
...
When(
  "dejo en blanco uno o más campos obligatorios del reporte de bug",
  async function () {
...
Then(
  "debería ver un mensaje de error indicando que todos los campos obligatorios deben completarse",
  async function () {
    // Espera a que el modal de error sea visible
    const errorModal = await driver.wait(
      until.elementLocated(By.css('[data-testid="no-fields-modal"]'))
    );
    await driver.wait(until.elementIsVisible(errorModal));
    // Encuentra el botón de cerrar dentro del modal
    const closeButton = await errorModal.findElement(
      By.xpath('//button[text()="Cerrar"]')
    );
    // Verifica que el modal esté presente
